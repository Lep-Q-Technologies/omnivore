...
export function useCreateNewsletterSubscription() {
  const [state, setState] = useState<{
    loading: boolean
    error: Error | null
  ...
  const createSubscription = useCallback(async (name: string) => {
    setState({ loading: true, error: null, data: null })
    try {
      const result = await graphqlRequest<{
        createNewsletterSubscription: NewsletterSubscriptionResult
      }>(CREATE_NEWSLETTER_SUBSCRIPTION_MUTATION, { name })
      setState({
        loading: false,
        error: null,
        data: result.createNewsletterSubscription,
      })

      return result.createNewsletterSubscription
    } catch (error) {
      const err =
        error instanceof Error
          ? error
          : new Error('Failed to create newsletter subscription')
      setState({ loading: false, error: err, data: null })
      throw err
    }
  }, [])
