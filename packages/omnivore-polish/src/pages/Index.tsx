...
const Index = () => {
  ...
  const [readLaterIds, setReadLaterIds] = useState<Set<string>>(new Set(['1', '3', '5']));
  ...
  const filteredArticles = useMemo(() => {
    let filtered = articles.filter((article) => {
      ...
      switch (activeTab) {
        ...
        case 'inprogress':
          return article.progress > 0 && article.progress < 100 && article.state !== 'archived';
        case 'readlater':
          return readLaterIds.has(article.id) && article.progress === 0 && article.state !== 'archived';
        case 'starred':
          return article.flairs.some(f => f.icon === 'star') && article.state !== 'archived';
        case 'trash':
          return article.state === 'archived';
        default:
          return true;
  ...
  const counts = useMemo(() => {
    return {
      ...
      inprogress: articles.filter(a => a.progress > 0 && a.progress < 100 && a.state !== 'archived').length,
      readlater: articles.filter(a => readLaterIds.has(a.id) && a.progress === 0 && a.state !== 'archived').length,
      starred: articles.filter(a => a.flairs.some(f => f.icon === 'star') && a.state !== 'archived').length,
  ...
  const handleClearSelection = () => {
    setArticles(prev => prev.map(article => ({ ...article, isSelected: false })));
  ...
  const handleArchive = (id?: string) => {
    if (id) {
      ...
      setArticles(prev => prev.map(article =>
        ids.includes(article.id) ? { ...article, state: 'archived', isSelected: false } : article
      ));
      setIsSelectMode(false);
      toast.success(`Moved ${ids.length} articles to Trash`, {
        action: {
          ...
          onClick: () => {
            setArticles(prev => prev.map(article =>
              ids.includes(article.id) ? { ...article, state: 'default' } : article
            ));
          },
        },
      });
  ...
  const handleStar = (id: string) => {
    setArticles(prev => prev.map(article => {
      ...
      if (hasStarFlair) {
        return { ...article, flairs: article.flairs.filter(f => f.icon !== 'star') };
      } else {
        return { ...article, flairs: [...article.flairs, { icon: 'star' as const, name: 'Starred' }] };
      }
    }));
  };
    ...
    if (id) {
      ...
      toast.success(`Deleted ${toDelete.length} articles permanently`, {
        action: {
          label: 'Undo',
          onClick: () => {
            setArticles(prev => [...prev, ...toDelete.map(a => ({ ...a, isSelected: false }))]);
          },
        },
      });
    }
  ...
  const handleRetry = (id: string) => {
    ...
    setTimeout(() => {
      setArticles(prev => prev.map(article =>
        article.id === id ? { ...article, state: 'default', readingTime: 5 } : article
      ));
            ...
            {filteredArticles.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h2 className="text-heading font-semibold text-foreground mb-2">
                  {activeTab === 'trash' ? 'Trash is empty' : 'No articles here'}
                </h2>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredArticles.map((article) => (
                    <LibraryCard
                      ...
                      onTagClick={(tag) => navigate(`/library?tag=${encodeURIComponent(tag.label)}`)}
                    />
                  ))}
                </AnimatePresence>
              </div>
